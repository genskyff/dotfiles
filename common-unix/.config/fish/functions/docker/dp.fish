...
function dp --description "List containers with status"
    ...
    set result (docker ps --format "{{.ID}} {{.Names}} {{.Status}}" -a $argv)
    if test $status -ne 0
        echo -n "$result"
        return 1
    end
    ...
    for container in $result
        echo $container | read -l container_id container_name container_status
    ...
    set all_containers $running_containers $exited_containers
